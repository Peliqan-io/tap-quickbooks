#!/usr/bin/env python3
"""
Baseline capture for tap-quickbooks — run once on Python 3.9.

Uses dev_mode (pre-issued access_token, no refresh) to avoid rotating prod's
refresh_token chain. The access_token expires ~1h after Intuit minted it, so
capture and the 3.11 regression run must both happen within that window.

Required env vars:
    TAP_QUICKBOOKS_CLIENT_ID
    TAP_QUICKBOOKS_CLIENT_SECRET
    TAP_QUICKBOOKS_REFRESH_TOKEN     (required by tap config, never used in dev_mode)
    TAP_QUICKBOOKS_ACCESS_TOKEN      (current; cat /etc/singer_config/connection_N/tap_config.json on prod)
    TAP_QUICKBOOKS_REALM_ID
    TAP_QUICKBOOKS_SANDBOX           (optional, default "false")
    TAP_QUICKBOOKS_START_DATE        (optional, default 2024-01-01T00:00:00Z)
"""
import json
import os
import subprocess
...
def main():
    ...
    include = os.getenv("TAP_QUICKBOOKS_INCLUDE_STREAMS", "accounts")
